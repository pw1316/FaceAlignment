...
inline string getImagePath(int i)
{
	char fname[256];
	string name;
	sprintf(fname, "pose_%d.jpg", i);
	return TRAIN_DATA_PATH + fname;
}
inline string getShapePath(int i)
{
	char fname[256];
	sprintf(fname, "pose_%d.land", i);
	return TRAIN_DATA_PATH + fname;
}
#else
...
#pragma omp parallel for reduction(+:nLeaves)
		for (int landmarkId = 0; landmarkId < NUM_LANDMARKS; ++landmarkId)
		{
			...
			sprintf(fileNameBuf, "s%02dl%02d.rf", stageId, landmarkId);
...
#pragma omp parallel for
		for (int coord = 0; coord < 2 * NUM_LANDMARKS; ++coord)
		{
			...
			char fileNameBuf[256];
			sprintf(fileNameBuf, "t%dl%d.w", stageId, coord);
			string fpath = TRAINED_DATA_PATH + fileNameBuf;
...
void LBF::test()
{
	...
	for (int i = 0; i < TRAIN_DATA_SIZE; ++i)
	{
		...
		dumpShapeBin(TEST_DATA_PATH + tshpname, shp);
		...
		for (int t = 1; t <= NUM_STAGES; ++t)
		{
			...
			feature_node* feas = genGlobalFeatures(nonzero_leaves, NUM_TREES*NUM_LANDMARKS);
